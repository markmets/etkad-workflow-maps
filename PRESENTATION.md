...
| **Workflow Galaxy** | a workflow | 240 workflows from 3 sources | which workflows resemble each other, and in what respect |
...
**Method Galaxy** places ETKAD's 13 workflows inside a larger, existing map,
because 13 workflows are not enough data to build a map from on their own. The
join is TaDiRAH, the method vocabulary HumAL already uses. Result: ETKAD uses
73 of 147 methods in the vocabulary; the other 74 are used elsewhere in Europe
...
- **Method Galaxy** and **Workflow Galaxy** both draw on the SSH Open
  Marketplace and the same 13 ETKAD workflows, but at different grain. Method
...
| ETKAD / HumAL | 13 | all of ETKAD's published workflows, in their English versions |
| **Total** | **240** | |
...
2. Scraped all 13 ETKAD workflow pages, reading controlled terms from the
...
2. Scraped the Programming Historian lesson index plus each lesson's ordered
   section headings.
3. Read all 13 ETKAD workflows, following the list's pagination: per-stage
   TaDiRAH tags from the Estonian page, and everything compared as words
   (titles, step text, subjects, materials) from the English version.
4. Built a 2,826-name software gazetteer from Marketplace tools and matched
...
ETKAD supplies 13 of 240 workflows (5% of the corpus) but 84 of the 297
per-stage method tags in the corpus (28% of the tagged evidence).

| Source | Workflows | Steps tagged in order |
|---|---|---|
| Programming Historian | 119 | 0 |
| SSH Open Marketplace | 108 | 213 (44% of them) |
| ETKAD / HumAL | 13 | 84 (every step of all 13) |

Per-stage method tagging is what makes "the shape of a workflow" computable.
Of the 48 workflows in the corpus with a usable shape, 13 are ETKAD's. On this
measure, ETKAD is the densest contributor in the corpus, not a small one.

Two things follow from this:

- Thirteen workflows are not a representative sample of Estonian digital
...
- **Step order predicts what a project does** (46.5% accuracy) — as good a
  predictor as wording and better than subject matter, using no knowledge of
  workflow content.
- **Shared software predicts little** (33% accuracy) — no better than the 34% obtained
...
- **Interpretation tends to happen early**, before analysis, and discovery
  tends to precede data capture. This pattern comes from 57 workflows that do
